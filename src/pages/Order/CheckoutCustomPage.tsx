...
const CheckoutCustomPage = () => {
  ...
  if (isLoading) return <div className="text-center py-10">Đang tải thông tin đơn hàng...</div>;
  if (isError || !data) return <div className="text-center text-red-500 py-10">Không thể tải đơn hàng!</div>;
    ...
    <div className="w-full max-w-2xl mx-auto px-6 py-10 mt-10 mb-20 bg-white rounded-xl shadow-lg">
        ...
        {data.quantity === 0 ? (
          ...
          <div className="divide-y border rounded-lg mb-2">
            <div className="flex items-center gap-4 py-3 px-2">
              <img
                src={data.designFileUrls[0] || "/placeholder.svg"}
              ...
              <div className="font-semibold text-right">{data.quotedPrice}đ</div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {addresses.length === 0 && (
            <div className="text-red-500 mt-1 text-sm">Bạn chưa có địa chỉ nào.</div>
          )}
        </div>
        ...
        <Button type="submit" className="w-full bg-black text-white py-2.5 text-base hover:bg-gray-800" disabled={addresses.length === 0}>
          Thanh toán
        </Button>
      </form>
      ...
      <ConfirmationModal
        ...
        details={{ name, phone, paymentMethod, expectedDeliveryDate }}
      />
    </div>
  );
};
