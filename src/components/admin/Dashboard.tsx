...
} from "@/components/admin-data/hooks/use-user";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchCountProducts = async () => {
      try {
        const count = await productApi.getAllProducts();
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      const mapped: User[] = res.map((u) => ({
        id: u.id,
        name: u.username ?? u.username ?? "Không tên",
        email: u.email,
  ...
  const totalCustomers = users.filter(
    (u) => u.role.toUpperCase() === "USER"
  ).length;

  useEffect(() => {
    const fetchRevenue = async () => {
      try {
        const data = await orderApi.getRevenue();
        setRevenue(data);
      } catch (err) {
