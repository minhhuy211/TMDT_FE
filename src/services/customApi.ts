...
export default {
  ...
  quoteOrderCustom: async (id: string, price: number): Promise<OrderCustomResponse> => {
    const token = localStorage.getItem("token");
    const payload = {
      id,
      price,
  ...
  rejectOrderCustom: async (id: string, status: string): Promise<OrderCustomResponse> => {
    const token = localStorage.getItem("token");

    // Kiểm tra trạng thái hợp lệ trước khi gọi API
    const validStatuses = ["PENDING_QUOTE", "QUOTED", "AWAITING_PAYMENT"];
    if (!validStatuses.includes(status)) {
      throw new Error("Đơn hàng này không thể từ chối vì trạng thái không hợp lệ.");
    }
    ...
    try {
      ...
      if (error.response) {
        ...
        if (code === 1012) {
          ...
          throw new Error("Đơn hàng không tồn tại hoặc đã có trạng thái không hợp lệ.");
        }
        throw new Error(message || "Đã xảy ra lỗi khi từ chối đơn hàng. Vui lòng thử lại.");
      } else {
        // Xử lý lỗi không xác định (kết nối mạng, v.v.)
        throw new Error("Lỗi không xác định. Vui lòng kiểm tra lại kết nối mạng.");
      }
    }
  }
}
